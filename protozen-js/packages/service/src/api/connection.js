...
import { rpcPath } from "@protozen/config/src/api/rpc-path";
...
export class Connection {
  ...
  constructor(options: {
    ...
    const { url, apikey, apitoken, timeout } = (this.options = defaultOptions(
      options
    ));
    this.#axios = create({
      baseURL: url,
  ...
  createService(Service: Object): Object {
    const servicePath = Service.__servicePath__;
    const rpcImpl = async (method, requestData, callback) => {
      const path = rpcPath(servicePath, method.name);
      const response = await new Promise(async (resolve) => {
        const response = await this.#axios
          .put(path, new Uint8Array(requestData))
          .catch((error) => {
